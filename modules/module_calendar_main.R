...
palette_sparkline = list("#ffffff","#EBAF4C","#95695D","#000000")
    ...
    tagList(
      lapply(LST_calendar_pre, function(x) {
        htmlTemplate(
          ...
          genus = x$Genus,
          species = x$Species,
          image_link = paste0("https:",gsub("((\\.jpeg)|(\\.JPG)|(\\.jpg)|(\\.png)|(\\.PNG)).*","\\1",str_remove((xml_attrs((read_html(x$Image_Link)%>%html_nodes("img"))[[1]])[["src"]]),"/thumb"))),
          #
